...
graphclass.default <- function(X = NULL, Y = NULL,
  ...
  if(type=="intersection") {
    ...
    gl = logistic_group_lasso_ridge(X, Y, D,
                                    ...
                                    MAX_ITER = MAX_ITER, 
                                    CONV_CRIT = CONV_CRIT, 
                                    MAX_TIME = MAX_TIME, folds= folds,lambda_selection=lambda_selection )
    ...
    gl_results$Yfit <-Yfit #exp(Yfit)/(1+exp(Yfit))
    gl_results$train_error <- sqrt(mean( (Yfit-Yoriginal)^2) ) 
    #print(gl_results$train_error)
    #print(gl_results$Yfit )
    ...
    if(!is.null(Xtest)) {
      ...
      if(!is.null(Ytest)) {
        gl_results$test_error =sqrt(mean ( (Yfit_test-Ytest)^2 ) )#predict.graphclass(gl_results, Xtest, type = "error", Ytest = Ytest)
      } 
    }
